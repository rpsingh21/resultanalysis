...
def getSessionAndsemester(request):
    ...
    if request.method == "POST":
        if sessionForm.is_valid():
            ...
            url_r = "/results/%s/%s/%s/%s/" % (college,
                                               branch, session, semester)
...
def showSemesterResult(request, college, branch, yearOfJoining, semester):
    ...
    subjects = Marks.objects.filter(
        student=results[0].student, subjectCode__semester=semester)
...
def showSubjectMarks(request, college, branch, yearOfJoining, semester, subjectCode):
    collegeObjects = get_object_or_404(College, collegeName=college)
    branchObjects = get_object_or_404(Branch, branchName=branch)
    subjectObject = get_object_or_404(
        Subject, subjectCode=subjectCode, semester=semester, branchCode=branchObjects)
    ...
    aggregations = results.filter(internal__gte=0, external__gte=0, totalMarks__gte=0).aggregate(Avg('internal'), Max('internal'), Min(
        'internal'), Avg('external'), Max('external'), Min('external'), Avg('totalMarks'), Max('totalMarks'), Min('totalMarks'))
    content = {
        'title': 'Result of Students',
        'results': results,
        'subjects': subjects,
...
def studentSemesterResult(request, rollNo, semester):
    ...
    subjectCodes = Subject.objects.filter(
        branchCode=student.branchCode, semester=semester)
...
def getResult(url):
    ...
    collegeCode = (
        ((soup.find(id='lblInstitute').text).split('(')[1]).split(')')[0]).strip()
    ...
    collegeObjects = College.objects.get_or_create(
        collegeCode=int(collegeCode))
    if collegeObjects[1]:
    ...
    branchObjects = Branch.objects.get_or_create(
        course=courseObjects[0], branchCode=int(branchCode))
    if branchObjects[1]:
    ...
    for table in tables:
        ...
        if tableId:
            ...
            if tableId.split('_')[-1:][0] == 'grdViewSubjectMarksheet':
                ...
                if semester != '0':
                    totalSubject = int(
                        (soup.find(id=topId+'lblTotalSubjectsCount').text).strip())
                    totalMarks = (
                        soup.find(id=topId+'lblSemesterTotalMarksObtained').text).strip()
                    totalInternal = 0
                    totalExternal = 0
                    ...
                    for i in range(0, totalSubject):
                        subjectCode = (
                            soup.find(id=tableId+'_subCode_'+str(i)).text).strip()
                        subjectName = (
                            soup.find(id=tableId+'_subName_'+str(i)).text).strip()
                        subjectType = (
                            soup.find(id=tableId+'_subType_'+str(i)).text).strip()
                        ...
                        # FIND MARKS
                        parentTd = (
                            soup.find(id=tableId+'_subCode_'+str(i))).parent.parent
                        ...
                        print(subjectCode, subjectInternal,
                              subjectExternal, backMarks)
                        ...
                        # SAVE SUBJECT MARKS
                        marksObject[0].totalMarks = int(
                            subjectInternal)+int(subjectExternal)
                        marksObject[0].save()

                        if subjectType != 'CA':
                            if subjectType == 'Theory':
                                totalInternal += int(subjectInternal)
                                totalExternal += max(int(subjectExternal),
                                                     int(backMarks))
                            elif subjectType == 'Practical':
                                totalInternal += (max(int(subjectExternal),
                                                  int(backMarks))+int(subjectInternal))
                            elif subjectType == 'GP':
                                totalInternal += int(subjectInternal)
...
def a():
    ...
    for student in students:
        ...
        for back in backs:
            newData = TotalMarks.objects.get(
                student=student, semester=back.subjectCode.semester)
